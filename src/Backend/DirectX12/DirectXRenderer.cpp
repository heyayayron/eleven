...
// from the upload heap to this
    ...
    DirectXRenderer::DirectXRenderer(const u32 width, const u32 height) : IRendererAPI() {
        ...
        _gfxContext = glfwCreateWindow(_width, _height, "Eleven (DirectX12)", NULL, NULL);
        _hwnd = glfwGetWin32Window(_gfxContext);
        _dc = GetDC(_hwnd);
    ...
    void DirectXRenderer::Setup(el::Vertex *vertices) {
        _vertices = vertices;
        OnInit();
    }
    ...
    void DirectXRenderer::Render() {
        MSG msg = {};
        while (!glfwWindowShouldClose(_gfxContext) && msg.message != WM_QUIT) {
            OnUpdate();
            OnRender();
            glfwPollEvents();
...
// Load the rendering pipeline dependencies.
    void DirectXRenderer::LoadPipeline() {
        if (_hwnd == NULL) throw std::exception("Eleven: DirectX hwnd is null");
...
// Render the scene.
    ...
    void DirectXRenderer::PopulateCommandList() {
        ...
        auto ntargets = CD3DX12_RESOURCE_BARRIER::Transition(m_renderTargets[m_frameIndex].Get(),
        ...
        view.StrideInBytes = sizeof(el::Vertex);
        ...
        auto xtargets = CD3DX12_RESOURCE_BARRIER::Transition(m_renderTargets[m_frameIndex].Get(),
                                                             D3D12_RESOURCE_STATE_RENDER_TARGET,
                                                             D3D12_RESOURCE_STATE_PRESENT);
